...
const MY_CONTRACT_ABI_PATH = "contract/abi/CommentVerifier.json"
var my_contract
var accounts
var web3

function metamaskReloadCallback(){
  window.ethereum.on('accountsChanges', (accounts) => {
    document.getElementById("web3_message").textContent = "Account changed..."
    window.location.reload()
  })
  window.ethereum.on('networkChanged', (accounts) => {
    document.getElementById('web3_message').textContent = "Network Changed..."
    window.location.reload()
  })
}
...
const onContractInitCallback = async() => {
  ...
  for(var i=parseInt(commentAmount); i > 0; i--){
    ...
    var titleElement = document.createTextNode("span")
...
const sendProof = async(title, text) => {
  document.getElementById("web3_message").textContent = "Please sign the message"
  ...
  var hashedMessage = etheres.utils.hashMessage(msgParams)
  const hashedMessageArray = etheres.utils.arrayify(hashedMessage)
